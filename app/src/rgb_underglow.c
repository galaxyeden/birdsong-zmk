...
#if IS_ENABLED(CONFIG_PM_DEVICE_RUNTIME) && DT_HAS_CHOSEN(skji_fireflies_argb_pwr)
static const struct device *argb_power = DEVICE_DT_GET(DT_CHOSEN(skji_fireflies_argb_pwr));
#endif // IS_ENABLED(CONFIG_PM_DEVICE_RUNTIME)) && DT_HAS_CHOSEN(skji_fireflies_argb_pwr)
...
static void zmk_rgb_underglow_effect_allofmystars(void) {
    uint8_t rando;
    static uint32_t randdata;
    randdata = sys_rand32_get();
    static uint8_t step24 = 0;
    uint8_t starspeed = 4;
...
static void zmk_rgb_underglow_effect_rainbowstars(void) {
    ...
    if (state.animation_step % starspeed == 0) {
        for (int i = 0; i < STRIP_NUM_PIXELS; i++) {
            ...
            struct zmk_led_hsb hsb = state.color;
            hsb.h = (((randdata >> step24) & 0xff) % 20) * 18;
            step24++;
            if (step24 >= 24) {
                randdata = sys_rand32_get();
                step24 = 0;
